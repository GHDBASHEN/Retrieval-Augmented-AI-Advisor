...
async def chat_with_bot(bot_id: int, query: ConversationQuery, db: AsyncSession = Depends(get_db)):
    # Verify bot exists
...
async def get_bot_history(bot_id: int, db: AsyncSession = Depends(get_db)):
